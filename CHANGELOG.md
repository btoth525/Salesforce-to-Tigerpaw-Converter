...
rewritten numbers and stray footer rows. Every one of those traced back to the
v1 pipeline guessing (encoding via `chardet`, types via `pandas`). v2 replaces
guessing with a deterministic, fully-logged engine.

### Fixed
- **Mojibake / "random letters".** Encoding is now detected deterministically:
  BOM → strict UTF-8 → Windows-1252 (never ISO-8859-1). A Windows-encoded
  export with `½` or a curly apostrophe no longer turns into `Ë` or `â€™`.
  Files that were *already* double-encoded by another tool are repaired.
- **Cells that vanished.** Part numbers or descriptions reading `NA`, `N/A`,
  `None`, `null` were silently blanked by pandas. Every cell is now a string;
  nothing is interpreted.
- **Numbers rewritten.** `2.00` became `2.0`, `00123` became `123`, `3E4`
  became `30000.0`. Values are now preserved exactly as exported.
- **Line breaks inside descriptions** were written back verbatim, splitting
  one product across two import rows. They are collapsed to a single space.
- **Salesforce footer** (`Copyright…`, `Confidential Information…`,
  `Generated By…`, `Grand Totals`) was imported as product rows. Dropped and
  reported.
- **UTF-16 exports** and **semicolon/tab-delimited** files failed with
  "Missing expected columns". Both parse now.
- **`docker stop` always SIGKILLed the app** (exit 137) because gunicorn ran
  under `sh -c` as PID 1. The container now stops gracefully.
- Unexpected decode errors returned an HTML 500 page; all errors are JSON.

### Added
- **Cleanup report** in the preview: encoding badge, every changed cell
  (before → after, reason), every dropped row (reason), and warnings such as
  non-numeric quantities or empty part numbers. Fixed cells are highlighted in
  the table with the original value on hover.
- **Header aliases**: `Qty`, `Unit Price`, `Sales Price`, `Part Number`,
  `Product: Product Code`, etc. are recognized and reported.
- **Header-row detection**: title/preamble lines above the header are skipped.
- **Output options** (persisted per browser): plain-ASCII text (default on),
  UTF-8 BOM (default on, v1 parity), quote every field, keep extra Salesforce
  columns (default on, v1 parity), map quote-line Group → Project Phase.
- **Number cleanup** on Quantity / Price / Cost: `$1,299.00` → `1299.00`,
  `USD 213.34` → `213.34`, `(12.50)` → `-12.50`. Decimals are left untouched.
- `X-Convert-Summary` response header; `_report.txt` inside batch ZIPs;
  version in `/api/health` and the UI footer.
- Preview/edit cap raised 500 → 2 000 rows.

### Removed
- `pandas` and `chardet` dependencies (image ~40% smaller, faster start).
